...
interface AuctionMarketplaceProps {
  onClose?: () => void;
  onAuctionSelected?: (auctionAddress: string, auctionImage?: string) => void;
}

export const AuctionMarketplace = ({ onClose, onAuctionSelected }: AuctionMarketplaceProps) => {
  ...
  // Registry contract address - loaded from JSON file
  const [REGISTRY_ADDRESS, setRegistryAddress] = useState<string>("");
  ...
  // Load registry address from JSON file
  useEffect(() => {
    const loadRegistryAddress = async () => {
      try {
        const registryData = await import('../contracts/AuctionRegistry.json');
        setRegistryAddress(registryData.address);
        console.log('📍 Loaded Registry address:', registryData.address);
      } catch (error) {
        console.error('❌ Failed to load Registry address:', error);
      } finally {
  ...
  useEffect(() => {
    if (ethersReadonlyProvider && REGISTRY_ADDRESS) {
  ...
  }, [ethersReadonlyProvider, REGISTRY_ADDRESS]);

  // Load auctions from Registry contract (on-chain only)
  useEffect(() => {
    if (registryContract && REGISTRY_ADDRESS) {
  ...
  const loadAuctionsFromRegistry = async (source = "unknown") => {
    if (!registryContract || !REGISTRY_ADDRESS) {
      console.log("❌ Registry contract not loaded or address not set, no fallback");
      return;
    }
  ...
  const createNewAuction = async () => {
    ...
    if (!REGISTRY_ADDRESS) {
      alert("Registry address not loaded. Please wait and try again.");
      return;
    }
        ...
        {isCreateFormOpen && (
          <div className="px-6 pb-6 border-t border-gray-200">
            ...
            <button
              ...
              disabled={isCreatingAuction || !newAuctionName.trim() || isLoadingRegistry}
              className={`mt-4 ${
                      ...
                      <button
                        onClick={() => {
                          ...
                          if (auction.contractAddress) {
                            // Call onAuctionSelected if provided
                            if (onAuctionSelected) {
                              onAuctionSelected(auction.contractAddress, auction.image || auction.imageUrl);
                            } else {
                              ...
                              window.dispatchEvent(new CustomEvent('auction-selected', {
                                detail: {
                                  ...
                                  auctionSeller: auction.seller || auction.creator,
                                  autoRefresh: true
                                }
                              }));
                            }
