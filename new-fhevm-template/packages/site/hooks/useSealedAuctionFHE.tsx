...
export const useSealedAuctionFHE = (parameters: {
  ...
  const canPlaceBid = useMemo(() => {
    ...
    console.log('[canPlaceBid] Debug:', {
      ...
      isRefreshing,
      isPlacingBid,
      stateIsBidding: state?.isBidding,
      canBid
    });
    ...
    (bidAmount: number) => {
      ...
      const run = async () => {
        ...
        try {
          ...
          // Use add64 for externalEuint64 (contract signature)
          input.add64(BigInt(bidAmount));
          
          // Add extraData for relayer (required by Sepolia relayer)
          const extraBytes = new TextEncoder().encode(JSON.stringify({
            v: 1,
            contract: thisAuctionAddress,
            user: thisEthersSigner.address,
            chainId: 11155111,
          }));

          const enc = await (input as any).encrypt({ extraData: extraBytes });
          ...
          // Contract only takes 2 parameters: encryptedBid and inputProof
