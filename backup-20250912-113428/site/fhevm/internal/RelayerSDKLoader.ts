...
// No longer using UMD - using ESM import from @zama-fhe/relayer-sdk
    ...
    return new Promise((resolve, reject) => {
      ...
      script.onload = () => {
        if (!isFhevmWindowType(window, this._trace)) {
          ...
          reject(
            new Error(
              `RelayerSDKLoader: Relayer SDK script has been successfully loaded, however, the window.relayerSDK object is invalid.`
            )
          );
        }
        resolve();
      ...
      script.onerror = () => {
        ...
        reject(
          new Error(
            `RelayerSDKLoader: Failed to load Relayer SDK`
          )
        );
      };
